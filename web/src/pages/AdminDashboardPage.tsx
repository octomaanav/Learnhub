...
function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-surface-100 overflow-hidden">
      <div>
        {/* Header */}
        <header className="bg-surface-50/80 backdrop-blur-sm border-b border-surface-200 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-surface-800">Admin Dashboard</h1>
                  <p className="text-xs text-surface-500">Manage content and settings</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <button
                  onClick={() => navigate('/pulse')}
                  className="p-3 rounded-xl border-2 transition-all font-bold font-display bg-surface-100 text-surface-600 border-surface-200 hover:bg-surface-200 hover:border-surface-300"
                  title="Open Pulse"
                >
                  Pulse
                </button>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Welcome Section */}
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-surface-800 mb-2">Welcome, Teacher!</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {adminFeatures.map((feature) => (
              <Link
                key={feature.path}
                to={feature.path}
                className="group relative bg-surface-50 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-surface-200 hover:border-transparent"
              >
                ...
                <div className="p-6">
                  ...
                  {/* Content */}
                  <h3 className="text-lg font-semibold text-surface-800 mb-2 group-hover:text-primary-600 transition-colors">
                    {feature.title}
                  </h3>
          ...
          {/* Coming Soon Section */}
          <div className="bg-surface-50 rounded-2xl shadow-sm border border-surface-200 p-6">
            <div className="flex items-center gap-2 mb-4">
              <svg className="w-5 h-5 text-tertiary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <h3 className="text-lg font-semibold text-surface-800">Coming Soon</h3>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {upcomingFeatures.map((feature, index) => (
                <div
                  key={index}
                  className="p-4 rounded-xl bg-surface-100 border border-surface-200 opacity-60"
                >
                  <div className="w-10 h-10 rounded-lg bg-surface-200 flex items-center justify-center text-surface-400 mb-3">
                    {feature.icon}
                  </div>
                  <h4 className="font-medium text-surface-700 text-sm mb-1">{feature.title}</h4>
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={index} className="bg-surface-50 rounded-xl p-4 border border-surface-200">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-lg">{stat.icon}</span>
                  <span className="text-2xl font-bold text-surface-800">{stat.value}</span>
                </div>
